...
// ── TEMPORARY DEBUG: test direct database access ──────────────────────────
async function debugNotionDatabase(dbId) {
  try {
    ...
    console.log('[crm-pipeline] NOTION DB TEST', {
      dbId,
      status: r.status,
    ...
    console.error('[crm-pipeline] NOTION DB TEST ERROR', e.message)
    return null
  }
}
...
function leadsDateFilter() {
  ...
  return {
    ...
    date: {
      on_or_after: d.toISOString().slice(0, 10)
    }
  }
}
...
const getFormula = p =>
  (
    p?.formula?.string ??
    (p?.formula?.number != null
      ? String(p.formula.number)
      : null)
  )

// Bucket the Lead Tier formula string into priority | average | weak
...
function computeStats({ pages, repMap, mStart, mEnd, now }) {
  ...
  for (const page of pages) {
    ...
    // Closed Won is bucketed by the date the deal actually closed,
    // NOT Submitted At.
    // Falls back to last_edited_time for records that predate the
    // Closed Won Date property.
    const wonDate =
      closedWonDateRaw
        ? new Date(closedWonDateRaw)
        : (isWon ? new Date(page.last_edited_time) : null)
    ...
    if (assigned.length > 0) {
      ...
      const repInfo =
        repMap[rid] ||
        repMap[rid.replace(/-/g, '')]

      if (repInfo) {
        // Only show rep in a month if they existed
        // before that month ended.
        const repCreated =
          repInfo.createdAt
            ? new Date(repInfo.createdAt)
            : null

        if (!repCreated || repCreated < mEnd) {
    ...
    if (inMonth) {
      ...
      stageCount[stageKey] =
        (stageCount[stageKey] || 0) + 1

      for (const prod of products) {
        productCount[prod] =
          (productCount[prod] || 0) + 1
      }
      ...
      if (status !== 'New Lead') {
        ...
        if (isLost) {
          ...
          repStats[repName].closedLost =
            (repStats[repName].closedLost || 0) + 1
        }
      }
    }

    // Closed Won counted independently of submission month
    // — by Closed Won Date instead.
    if (wonInMonth) {
      closedWon++

      repStats[repName].closedWon++

      if (source) {
        sourceClosedCount[source] =
          (sourceClosedCount[source] || 0) + 1
      }

      if (submDate) {
        daysToCloseSum +=
          (wonDate - submDate) / 86400000
      ...
      if (quoSentDate) {
        quoteToWinSum +=
          (wonDate - new Date(quoSentDate)) / 86400000
    ...
    if (nextFU && !isClosed) {
      if (nextFU <= today) {
        followupsToday++
        repStats[repName].followupsToday++
      }
  ...
  const avgDaysToClose =
    daysToCloseCount > 0
      ? Math.round(
          daysToCloseSum / daysToCloseCount
        )
      : null
  ...
  const avgQuoteToWin =
    quoteToWinCount > 0
      ? Math.round(
          quoteToWinSum / quoteToWinCount
        )
      : null

  const tierPct = n =>
    monthLeads > 0
      ? Math.round((n / monthLeads) * 100)
      : 0
    ...
    tiers: [
      {
        key: 'priority',
        label: 'Priority',
        count: tierCount.priority,
        pct: tierPct(tierCount.priority)
      },
      {
        key: 'average',
        label: 'Average',
        count: tierCount.average,
        pct: tierPct(tierCount.average)
      },
      {
        key: 'weak',
        label: 'Weak',
        count: tierCount.weak,
        pct: tierPct(tierCount.weak)
      }
    ]
  }
      ...
      .then(([pages, people]) => {
        ...
        for (const person of people) {
          const nameProp =
            ...
            person.properties['Nama'] ||
            person.properties['Full Name']
          ...
          // Role must be exactly 'Sales Rep'
          const roleArr =
            getMultiSel(roleProp)
          ...
          const statusProp =
            ...
            person.properties['Active'] ||
            person.properties['Employment Status']

          const personStatus =
            getStatus(statusProp)

          if (
            !personStatus ||
            personStatus.toLowerCase() !==
              'active'
          ) {
            continue
          }
...
export default async function handler(req, res) {
  ...
  if (!NOTION_KEY) {
    return res.status(500).json({
      error:
        'Server misconfigured: NOTION_API_KEY not set'
    })
  }

  // TEMPORARY DEBUG:
  // Verify whether the token can directly access
  // the Enquiry database.
  await debugNotionDatabase(ENQUIRY_DB)
  ...
  if (hit && hit.stale) {
    ...
    buildFetchPromise(ck)
      .catch(e =>
        console.error(
          '[crm-pipeline] bg refresh failed:',
          e.message
        )
      )
  ...
  try {
    ...
    res.status(503).json({
      error:
        'Notion API unavailable and no cache exists. Try again shortly.'
    })
  }
}
